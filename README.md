...
| `--total-timesteps` **INT** | total timesteps of the experiments **`(default: 1,000,000)`** |
...
| `--encoder-lr` **FLOAT** | the learning rate of the encoder network optimizer **`(default: 0.0003)`** |
| `--decoder-lr` **FLOAT** | the learning rate of the decoder network optimizer **`(default: 0.0003)`** |
| `--weight-lambda` **FLOAT** | decoder parameters weight decay coefficient **`(default: 0.0000001)`** |
| `--latent-lambda` **FLOAT** | L2 regularization coefficient **`(default: 0.000001)`** |
...
- Petros Christodoulou. Soft actor-critic for discrete action settings. arxiv 2019. arXiv preprint arXiv:1910.07207.
...
- Nino Vieillard, Olivier Pietquin, and Matthieu Geist. Munchausen reinforcement learning, 2020.
...
- Lili Chen, Kimin Lee, Aravind Srinivas, and Pieter Abbeel. Improving computational efficiency in visual reinforcement learning via stored embeddings, 2021.
...
- Tristan Billot. How fast is mlx? a comprehensive benchmark on 8 apple silicon chips and 4 cuda gpus. Towards Data Science, February 2024.
- The Centre for Computing History. Space invaders. https://www.computinghistory.org.uk/det/329/Space-Invaders/. Webpage describing the Atari 2600 game Space Invaders. Reference ID CH329. Access Date: 2025-04-14.
- Atari, Inc. Space Invaders Instruction Manual, 1980. Hosted on AtariAge. Accessed: 2025-04-14.