...
from utils import choose_device, vector_env_setup, save_model, upload_videos, Model, get_episodic_stats, update_target_networks, upload_model
from networks import SoftQNetwork, Actor, Encoder, Decoder
...
# ================ SAC+AE specific arguments ================
    encoder_lr: float = 0.0003
    """the learning rate of the encoder network optimizer"""
    decoder_lr: float = 0.0003
    """the learning rate of the decoder network optimizer"""
    weight_lambda: float = 0.0000001
    """decoder parameters weight decay coefficient"""
    latent_lambda: float = 0.000001
...
# ================ SETUP ================
    ...
    encoder_target = Encoder(envs).to(device)
    decoder = Decoder(envs, encoder.feature_dim, encoder.output_dim).to(device)
    actor = Actor(envs, encoder).to(device)
    qf1 = SoftQNetwork(envs, encoder).to(device)
    qf2 = SoftQNetwork(envs, encoder).to(device)
    qf1_target = SoftQNetwork(envs, encoder_target).to(device)
    qf2_target = SoftQNetwork(envs, encoder_target).to(device)
    ...
    decoder_optimizer = torch.optim.Adam(decoder.parameters(), lr=args.decoder_lr, weight_decay=args.weight_lambda)
    q_parameters = list(qf1.fc1.parameters()) + list(qf1.fc2.parameters()) + list(qf1.fc_q.parameters()) + list(qf2.fc1.parameters()) + list(qf2.fc2.parameters()) + list(qf2.fc_q.parameters()) + list(encoder.parameters())
    q_optimizer = optim.Adam(q_parameters, lr=args.q_lr, eps=1e-4)
    actor_optimizer = optim.Adam(list(actor.fc1.parameters()) + list(actor.fc2.parameters()) + list(actor.fc_logits.parameters()), lr=args.policy_lr, eps=1e-4)
...
# ================ Training loop ================
    ...
    # ================ Data collection ================
        ...
        if global_step < args.learning_starts:
            ...
            actions, log_prob, action_probs, entropy = actor.get_action(torch.Tensor(obs).to(device))
    ...
    # ================ Training ================
        ...
        # ================ Critic training ================
                ...
                if args.double_avg_w_q_clip:
                    # Q-clipping (SD-SAC)
                    with torch.no_grad():
                        qf1_old_target = qf1_target(data.observations)
                        qf2_old_target = qf2_target(data.observations)
        ...
        # ================ AutoEncoder training ================
                # https://github.com/denisyarats/pytorch_sac_ae/blob/master/sac_ae.py#L369
                latent_vector = encoder(data.observations)
                reconstructed_obs = decoder(latent_vector)
                reconstruction_loss = F.mse_loss(reconstructed_obs, data.observations / 255.0)
                latent_loss = (0.5 * latent_vector.pow(2).sum(dim=1)).mean()    # L2 loss
                ae_loss = reconstruction_loss + args.latent_lambda * latent_loss
        ...
        # ================ Update target networks ================
            if (global_step // args.target_network_frequency) > (previous_step // args.target_network_frequency):
                update_target_networks(qf1, qf1_target, args.tau, False)
                update_target_networks(qf2, qf2_target, args.tau, False)
                update_target_networks(encoder, encoder_target, args.tau, True)
        ...
        # ================ Performance metrics logging ================
            ...
            if global_step % 100 < args.num_envs:
                ...
                writer.add_scalar('losses/autoencoder_loss', ae_loss.item(), global_step)
